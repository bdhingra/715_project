...
def char2word2vec(batch, mask, params, n_char=N_CHAR):
    ...
    # sequence embeddings
    seq_emb, w2s_net = word2seq(seqs,seqs_mask,params,'w2s_')

    return seq_emb, c2w_net, w2s_net

def word2seq(seqs,mask,params,prefix):
    '''
    ...
    l_fgru_source = lasagne.layers.GRULayer(l_in_source, W2S_HDIM, resetgate=c2w_f_reset, updategate=c2w_f_update, hidden_update=c2w_f_hidden, hid_init=lasagne.init.Constant(0.), backwards=False, learn_init=True, gradient_steps=-1, grad_clipping=0, unroll_scan=False, precompute_input=True, mask_input=l_mask)
    ...
    l_bgru_source = lasagne.layers.GRULayer(l_in_source, W2S_HDIM, resetgate=c2w_b_reset, updategate=c2w_b_update, hidden_update=c2w_b_hidden, hid_init=lasagne.init.Constant(0.), backwards=True, learn_init=True, gradient_steps=-1, grad_clipping=0, unroll_scan=False, precompute_input=True, mask_input=l_mask)
    ...
    # Dense layer
    l_fdense_source = lasagne.layers.DenseLayer(l_f_source, SDIM, W=params[prefix+'W_df'], b=params[prefix+'b_df'], nonlinearity=None)
    l_bdense_source = lasagne.layers.DenseLayer(l_b_source, SDIM, W=params[prefix+'W_db'], b=params[prefix+'b_db'], nonlinearity=None)
...
def char2word(seqs,mask,params,prefix,n_char=N_CHAR):
    ...
    # lookup
    l_clookup_source = lasagne.layers.EmbeddingLayer(l_in_source, input_size=n_char, output_size=CHAR_DIM, W=params[prefix+'Wc'])
    ...
    # Dense layer
    l_fdense_source = lasagne.layers.DenseLayer(l_f_source, WDIM, W=params[prefix+'W_df'], b=params[prefix+'b_df'], nonlinearity=None)
    l_bdense_source = lasagne.layers.DenseLayer(l_b_source, WDIM, W=params[prefix+'W_db'], b=params[prefix+'b_db'], nonlinearity=None)
...
def init_params_c2w2s(n_chars=N_CHAR):
    ...
    # lookup table
    params[prefix+'Wc'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(n_chars,CHAR_DIM)).astype('float32'), name=prefix+'Wc')

    # f-GRU
    params[prefix+'W_f_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(CHAR_DIM,C2W_HDIM)).astype('float32'), name=prefix+'W_f_r')
    params[prefix+'W_f_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(CHAR_DIM,C2W_HDIM)).astype('float32'), name=prefix+'W_f_z')
    params[prefix+'W_f_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(CHAR_DIM,C2W_HDIM)).astype('float32'), name=prefix+'W_f_h')
    params[prefix+'b_f_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM)).astype('float32'), name=prefix+'b_f_r')
    params[prefix+'b_f_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM)).astype('float32'), name=prefix+'b_f_z')
    params[prefix+'b_f_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM)).astype('float32'), name=prefix+'b_f_h')
    params[prefix+'U_f_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,C2W_HDIM)).astype('float32'), name=prefix+'U_f_r')
    params[prefix+'U_f_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,C2W_HDIM)).astype('float32'), name=prefix+'U_f_z')
    params[prefix+'U_f_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,C2W_HDIM)).astype('float32'), name=prefix+'U_f_h')

    # b-GRU
    params[prefix+'W_b_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(CHAR_DIM,C2W_HDIM)).astype('float32'), name=prefix+'W_b_r')
    params[prefix+'W_b_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(CHAR_DIM,C2W_HDIM)).astype('float32'), name=prefix+'W_b_z')
    params[prefix+'W_b_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(CHAR_DIM,C2W_HDIM)).astype('float32'), name=prefix+'W_b_h')
    params[prefix+'b_b_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM)).astype('float32'), name=prefix+'b_b_r')
    params[prefix+'b_b_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM)).astype('float32'), name=prefix+'b_b_z')
    params[prefix+'b_b_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM)).astype('float32'), name=prefix+'b_b_h')
    params[prefix+'U_b_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,C2W_HDIM)).astype('float32'), name=prefix+'U_b_r')
    params[prefix+'U_b_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,C2W_HDIM)).astype('float32'), name=prefix+'U_b_z')
    params[prefix+'U_b_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,C2W_HDIM)).astype('float32'), name=prefix+'U_b_h')

    # dense
    params[prefix+'W_df'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,WDIM)).astype('float32'), name=prefix+'W_df')
    params[prefix+'W_db'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(C2W_HDIM,WDIM)).astype('float32'), name=prefix+'W_db')
    params[prefix+'b_df'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM)).astype('float32'), name=prefix+'b_df')
    params[prefix+'b_db'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM)).astype('float32'), name=prefix+'b_db')

    prefix = 'w2s_'

    # f-GRU
    params[prefix+'W_f_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM,W2S_HDIM)).astype('float32'), name=prefix+'W_f_r')
    params[prefix+'W_f_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM,W2S_HDIM)).astype('float32'), name=prefix+'W_f_z')
    params[prefix+'W_f_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM,W2S_HDIM)).astype('float32'), name=prefix+'W_f_h')
    params[prefix+'b_f_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM)).astype('float32'), name=prefix+'b_f_r')
    params[prefix+'b_f_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM)).astype('float32'), name=prefix+'b_f_z')
    params[prefix+'b_f_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM)).astype('float32'), name=prefix+'b_f_h')
    params[prefix+'U_f_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,W2S_HDIM)).astype('float32'), name=prefix+'U_f_r')
    params[prefix+'U_f_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,W2S_HDIM)).astype('float32'), name=prefix+'U_f_z')
    params[prefix+'U_f_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,W2S_HDIM)).astype('float32'), name=prefix+'U_f_h')

    # b-GRU
    params[prefix+'W_b_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM,W2S_HDIM)).astype('float32'), name=prefix+'W_b_r')
    params[prefix+'W_b_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM,W2S_HDIM)).astype('float32'), name=prefix+'W_b_z')
    params[prefix+'W_b_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(WDIM,W2S_HDIM)).astype('float32'), name=prefix+'W_b_h')
    params[prefix+'b_b_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM)).astype('float32'), name=prefix+'b_b_r')
    params[prefix+'b_b_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM)).astype('float32'), name=prefix+'b_b_z')
    params[prefix+'b_b_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM)).astype('float32'), name=prefix+'b_b_h')
    params[prefix+'U_b_r'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,W2S_HDIM)).astype('float32'), name=prefix+'U_b_r')
    params[prefix+'U_b_z'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,W2S_HDIM)).astype('float32'), name=prefix+'U_b_z')
    params[prefix+'U_b_h'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,W2S_HDIM)).astype('float32'), name=prefix+'U_b_h')

    # dense
    params[prefix+'W_df'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,SDIM)).astype('float32'), name=prefix+'W_df')
    params[prefix+'W_db'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(W2S_HDIM,SDIM)).astype('float32'), name=prefix+'W_db')
    params[prefix+'b_df'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(SDIM)).astype('float32'), name=prefix+'b_df')
    params[prefix+'b_db'] = theano.shared(np.random.uniform(low=-SCALE, high=SCALE, size=(SDIM)).astype('float32'), name=prefix+'b_db')
